...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeDineroAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDineroAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeDineroAmount = strAnonymizeDineroAmount.remove(strAnonymizeDineroAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDineroAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeDineroAmount * COIN) nMaxToAnonymize = nAnonymizeDineroAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeDineroAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeDineroAmount));
        strAnonymizeDineroAmount = strAnonymizeDineroAmount.remove(strAnonymizeDineroAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDineroAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
